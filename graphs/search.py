...
def aStar(G: nx.DiGraph, climber: Climber, start_id: int, end_id: int) -> Optional[List[int]]:
    start_state = (start_id, start_id)
    #heap: (f = g + h, g = cummulative weights, current state , path_seqeunce to return)
    pq = [(get_dist(G, start_id, end_id),0,start_state, [start_id])]
